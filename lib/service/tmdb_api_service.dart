...
class TmdbApiService {
  ...
  Future<List<TvListResultObject>> getPopularTvShows() async {
    try {
      Response response = await _dio.get('$_baseUrl/tv/popular?api_key=$_apiKey');
  ...
  Future<List<TvListResultObject>> getDiscoverTv(int year, {int page = 1}) async {
    try {
      Response response = await _dio.get('$_baseUrl/discover/tv?&with_genres=$_animeGenre&air_date.gte=$year-01-01&air_date.lte=$year-12-31&with_original_language=ja&language=ja&api_key=$_apiKey&page=$page');
  ...
  Future<TvDetailResultObject?> getTv(int tvId) async {
    try {
      Response response = await _dio.get('$_baseUrl/tv/$tvId?api_key=$_apiKey&language=ja');
  ...
  Future<TvWatchProviderResultObject?> getTvProvider(int tvId) async {
    try {
      Response response = await _dio.get('$_baseUrl/tv/$tvId/watch/providers?api_key=$_apiKey&language=ja');
  ...
  Future<List<TvListResultObject>?> getTvRecommendations(int tvId) async {
    try {
      Response response = await _dio.get('$_baseUrl/tv/$tvId/recommendations?api_key=$_apiKey&language=ja');
      List<TvListResultObject> list = response.data['results'].map((e) {
        return TvListResultObject.fromJson(e);
      }).toList();
  ...
  Future<AggregateCreditObject?> getAggregateCredits(int tvId) async {
    try {
      Response response = await _dio.get('$_baseUrl/tv/$tvId/aggregate_credits?api_key=$_apiKey&language=ja');
